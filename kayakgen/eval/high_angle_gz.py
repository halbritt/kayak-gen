"""High-angle GZ read-model builder (RFC 0043, stage 1+).

This module assembles the opt-in ``high_angle_gz`` JSON block used by:

- ``kayakgen stability --high-angle-gz`` (CLI surface; RFC 0043 stage 1).
- The sweep evaluator (``kayakgen/search/sweep.py``; RFC 0043 stage 2).
- The comparison report read model (``kayakgen/search/compare.py``;
  RFC 0043 stage 3).
- The Trame web workspace read model (``kayakgen/ui/web/read_models.py``;
  RFC 0043 stage 3).

The block is a display-only read model backed by ``evaluate_gz_curve``; the
underlying evaluator return type is unchanged so default ``kayakgen
stability`` output is byte-equal to prior behavior when the flag is absent.
...
``sealed_body_assumption``. These are emitted by the surface; the
evaluator-level warning set is preserved and merged in as well.

Phase 2 of ``ARCHITECTURE_RECOMMENDATION_PLAN_2026-05-16.md`` moved this
module from ``kayakgen.cli`` to ``kayakgen.eval`` so that the search and
sweep layers (and any future evaluator-side consumer) no longer reach into
the CLI layer for read-model assembly. ``kayakgen.cli.high_angle_gz`` now
re-exports these names as a thin compatibility shim.
"""
...
def _summary_block(
    curve: GeneratedBodyGZCurve, heel_grid_deg: Sequence[float]
) -> dict[str, Any] | None:
...
def build_high_angle_gz_block(
    ...
    *,
    ...
    """Assemble the opt-in ``high_angle_gz`` block for ``hull``.
