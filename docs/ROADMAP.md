...
| Stability calibration acceptance | RFC 0058 landed stages 1-3: stability fit schemas, `resolve_analytical_claim_label`, `cfd_in_loop_evaluator_status`, schema-only `kayakgen stability` fixture/fit commands, Generate-panel frontier colour wiring, and form-builder acknowledgement visibility wiring. Defaults remain byte-stable with an empty fit registry. | `landed` | Stage 4 first promotion remains gated on D007/D014 physical rig data. No fixture or fit is promoted, and RFC 0043 high-angle GZ remains `unvalidated_hydrostatic_comparison` until measured rig data and an accepted fit exist. |
| Generative-search jobs and web Generate panel | RFC 0057 stages 1-4 landed (cowboy 2026-05-17 → 2026-05-19): `GenerativeJob` records + progress sink + in-process and subprocess managers + `kayakgen runs jobs` CLI + eight `/api/generative-jobs/*` routes + Trame Generate panel with form-builder primary input, claim-admissibility-filtered objective picklist, RFC 0046 CFD-in-loop opt-in row, 2D Pareto scatter + sortable table with candidate handoff and undo toast, 1 s / 10 s auto-poll, one-click fork-with-new-seed, log redaction, and a subprocess-by-default `kayakgen serve` (`--jobs-in-process` opt-in). `forked_from` field on `GenerativeJob` records lineage. | `landed` | Future RFCs may add SSE/WebSocket push for progress, 3D scatter for 4+ objectives, a seed-sweep affordance, or CFD-in-loop graduation from opt-in to first-class once a measured-stability dataset arrives. The stage-4 workflow scaffold lives at `docs/workflows/0054-rfc-0057-stage-4-ui-polish/`. |
| Code+doc audit cadence | RFC 0059 landed 2026-05-22 as the `code_doc_audit` workflow shape — three independent lanes (pipeline-integrity / docs-decision-drift / operator-adoption) → synthesis → remediation plan, runnable workflow at `docs/workflows/0029-code-doc-audit/`. First dogfood run under `docs/audits/2026-05-22-code-doc-audit/` produced 13 findings; follow-up workflows 0030-0033 closed every high/medium item. D041 records the standing cadence (full_repo quarterly + release_candidate before any public-CLI / public-schema CHANGELOG entry). | `landed` | Future audits use this shape; per RFC 0059 §6 the presets `rfc_cluster` / `release_candidate` / `subsystem` / `adoption_path` cover narrower scopes. Lane-diversity (claude / codex / gemini) is recommended but not required. |
| Web Generate-panel form labels | RFC 0060 landed 2026-05-22 as `HullParameterMetadata` value object + 11-row registry under `kayakgen/ui/parameter_metadata.py`. The Trame Generate panel renders friendly labels + hover-for-description tooltips for every hull parameter and sources objective-picklist labels from the existing `OBJECTIVE_METADATA`. Submission payload is byte-stable. Closes audit finding AUD-O-003 via workflow `docs/workflows/0033-web-generate-panel-labels/`. | `landed` | Desktop `SLIDERS` migration to the same registry remains a deferred follow-up; objective-metric descriptions could later land on `ObjectiveMetadata` directly (D043 open question). |
