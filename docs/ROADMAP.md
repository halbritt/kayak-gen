...
| Stability calibration acceptance | RFC 0058 stage 1 landed as schema-only plumbing: `FixtureRef`, `HullFamilyScope`, `StabilityFitMetrics`, `ReviewerSignature`, `StabilityFitRecord`, and `StabilityFixturePromotionPacket` under `kayakgen/eval/stability/accepted_fit.py`, with default fit-threshold validators and promotion-packet gates. | `partial` / `evidence-gated` | Stage 2 still owns `resolve_analytical_claim_label` and `cfd_in_loop_evaluator_status`; stage 3 owns the `kayakgen stability` sub-app and read-model wiring. No fixture or fit is promoted, and RFC 0043 high-angle GZ remains `unvalidated_hydrostatic_comparison` until measured rig data and an accepted fit exist. |
