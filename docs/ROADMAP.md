...
| Geometry and mesh evidence | RFCs 0021-0023 and 0028 landed diagnostics, generated-body, fixture-handoff, and plumb-stem safe slices; workflow 0051 landed a readiness report; cowboy 2026-05-15 landed RFC 0040 generated-body parameter-matrix hardening (55-case test surface) and the `snappyHexMesh` evidence-harness contract (`kayakgen/eval/snappy_hex_mesh.py`) with deterministic dictionary scaffolds, `CheckMeshSummary`, and the partial-evidence-safe diagnostic translator. No new `cfd_ready` promotion path. | `partial` / `evidence-gated` | Render real meshing cases under `openfoam-v2512-snappyhexmesh-watertight-v1` and bind body refs + dict hashes + v2512 provenance + `checkMesh` + artifact checksums into the existing `SnappyHexMeshEvidence` contract. Real `cfd_ready` promotion still gated on full evidence binding. |
...
Status: `partial` / `evidence-gated` (steps 1-2 landed 2026-05-15)
...
Status: `partial` / `evidence-gated` (stages 1-3 landed 2026-05-15; stage 4 desktop pending)
