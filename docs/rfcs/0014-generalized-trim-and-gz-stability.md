...
generalized trim and high-angle stability open. `kayakgen.eval.stability`
currently has no longitudinal load model and deliberately does not claim a
computed `GZCurve`.
...
The result model adds:

```python
TrimEquilibriumResult(
    draft_at_midship_m: float,
    sinkage_m: float,
    trim_angle_deg: float,
    load_lcg_m: float,
    buoyancy_lcb_m: float,
    displacement_error_kg: float,
    moment_error_kg_m: float,
    converged: bool,
    iterations: int,
    warnings: list[str],
)
...
solver-specific CFD package. If that body is unavailable, `evaluate_gz_curve`
must return an unavailable/not-implemented status with a clear reason instead
of synthetic righting arms.

The `GZCurve` contract remains:
...
- Load cases round-trip with explicit longitudinal load components.
- Compact legacy load fields normalize to the component model without changing
  existing default equilibrium-sinkage output.
- A forward LCG produces bow-down trim and an aft LCG produces stern-down trim
  under otherwise equal inputs.
- Trim solve results include displacement and moment residuals, convergence
  status, iteration count, and warnings.
- CLI output includes trim fields in equilibrium mode.
- Sweep/evaluation records can carry trim equilibrium output without breaking
  existing JSON consumers.
- `evaluate_gz_curve` emits real `GZ` values only when a named closed-volume
  model is used.
- Tests prove that unsupported high-angle stability remains unavailable rather
  than producing placeholder curves.
...
- Step 4 - Define the first closed-volume stability body in code and docs, or
  keep `GZCurve` unavailable with explicit warnings.
- Step 5 - Implement high-angle `GZCurve` only after the volume decision has
  landed.

## Domain Modeling

`LongitudinalLoadComponent` and `LoadCase` are value objects. `TrimEquilibrium`
and `GZCurve` are evaluator read models derived from the `Hull` aggregate and a
load case.
